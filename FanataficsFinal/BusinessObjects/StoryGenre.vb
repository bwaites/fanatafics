...
#Region " Private Members "
    Private _GenreID As Guid = Guid.Empty
...
#Region " Private Methods "
    Private Function Insert(database As DatabaseHelper.Database) As Boolean
        ...
        Try
            ...
            database.Command.CommandText = "tblStoryGenre_INSERT"
            'Add the header data parameters
            MyBase.Initialize(database, Guid.Empty)
            'Add the parameter
            database.Command.Parameters.Add("@GenreID", SqlDbType.UniqueIdentifier).Value = _GenreID
    ...
    Private Function Update(database As DatabaseHelper.Database) As Boolean
        ...
        Try
            ...
            database.Command.Parameters.Clear()
            database.Command.CommandType = CommandType.StoredProcedure
            database.Command.CommandText = "tblStoryGenre_UPDATE"
            'Add the header data parameters
    ...
    Private Function Delete(database As DatabaseHelper.Database) As Boolean
        ...
        Try
            database.Command.Parameters.Clear()
            database.Command.CommandType = CommandType.StoredProcedure
            database.Command.CommandText = "tblStoryGenre_DELETE"
            MyBase.Initialize(database, MyBase.Id)
            database.ExecuteNonQueryWithTransaction()
            MyBase.Initialize(database.Command)
...
#Region " Public Methods "
    ...
    Public Function IsSavable() As Boolean
        ...
        End If
    End Function
    Public Function GetById(id As Guid) As StoryGenre

        Dim db As New Database(My.Settings.ConnectionName)
        Dim ds As DataSet = Nothing
        db.Command.CommandType = CommandType.StoredProcedure
        db.Command.CommandText = "tblStoryGenre_getById"
        ...
        Else
            If ds.Tables(0).Rows.Count = 0 Then
                Throw New Exception(String.Format("StoryGenre {0} was not fount", id))
            Else
                Throw New Exception(String.Format("StoryGenre {0} found multiple records", id))
            End If
        End If
